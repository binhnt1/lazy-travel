...
fun TourNormalSection(
    ...
    // Load normal tours (tours without HOT/LUXURY/FEATURED badges)
    LaunchedEffect(Unit) {
        scope.launch {
            tourRepo.getRecords<Tour>(
                ...
                onSuccess = { fetchedTours ->
                    // Filter out tours with special badges
                    tours = fetchedTours.filter { tour ->
                        tour.badges?.none {
                            it.contains("HOT", ignoreCase = true) ||
                            it.contains("LUXURY", ignoreCase = true) ||
                            it.contains("FEATURED", ignoreCase = true)
                        } != false && !tour.featured
                    }.take(15)
    ...
    // Normal section with white card background
    Card(
        modifier = modifier
            .fillMaxWidth()
        ...
        Column(
            ...
            // Normal tours horizontal scroll
            when {
                isLoading -> {
                    Box(
                        modifier = Modifier
